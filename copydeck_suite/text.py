...
VERSION = "2.3.1 [Questionary + 'My Tone' Edition]"
...
def init_config() -> Tuple[configparser.ConfigParser, str]:
    """
    Check if we already have prx.ini in G:\tools\.
    ...
    if os.path.exists(config_path):
        ...
        # Let's create a brand new config
...
def setup_logging() -> None:
    """
    If user wants logs, let's configure them. Otherwise, it's just console logs.
    """
    ...
    try:
        ...
        console.print(
            Panel(
                f"Couldn't make log dir '{log_dir}', so we'll log to console only",
                style="red",
            )
        )
        log_dir = ""
...
def is_file_damaged(
    ...
    """
    Decide if a local file is incomplete/damaged.
    We try to see if local_size < (remote_size - tolerance).
    If we can't figure out remote_size, we assume it's fine.
    """
...
def download_podcast_rss(
    ...
    if not to_download:
        console.print(
            Panel(
                "No episodes left to download after sorting/filtering.", style="yellow"
            )
        )
        return
    ...
    for idx, ep in enumerate(to_download, start=1):
        ...
        if os.path.exists(file_path):
            ...
            if not damaged:
                console.print(
                    Panel(
                        f"SKIPPING: '{file_path}' already looks complete.",
                        style="yellow",
                    )
                )
        ...
        if success:
            console.print(
                Panel(f"[green]✔ Downloaded[/green] '{title}'", style="green")
            )
        else:
            console.print(
                Panel(f"[red]❌ Failed to download[/red] '{title}'", style="red")
            )
    ...
    if FAILED_DOWNLOADS:
        ...
        console.print(
            Panel(f"The following downloads failed:\n{fail_list}", style="red")
        )
...
def manage_podcasts_in_config() -> None:
    ...
    while True:
        choice = questionary.select(
            "Manage Podcasts:",
            choices=["View list", "Add new", "Edit existing", "Remove", "Return"],
            style=custom_style,
        ).ask()
        ...
        if choice == "View list":
            ...
            new_val = questionary.text(
                "Enter: LINK : NAME_ID : OUTPUT_DIR", style=custom_style
            ).ask()
            if not new_val:
                continue
            ...
            with open(config_path, "w") as cf:
                config.write(cf)
            console.print(
                Panel(f"Removed '{removed[1]}' from the list.", style="green")
            )
...
def browse_local_files() -> None:
    ...
    sel = questionary.select(
        "Pick a podcast to see its local mp3s:", choices=choices, style=custom_style
    ).ask()
    if not sel:
        return
    ...
    if not os.path.exists(out_dir):
        console.print(
            Panel(f"'{out_dir}' doesn't exist, can't browse it.", style="red")
        )
        return
...
def manage_settings_config() -> None:
    ...
    if action == "View prx.ini":
        ...
        name = questionary.text(
            "User name:", default=config["user"].get("name", ""), style=custom_style
        ).ask()
        ...
        new_width = questionary.text(
            "Console window width:", default=width, style=custom_style
        ).ask()
        ...
        new_height = questionary.text(
            "Console window height:", default=height, style=custom_style
        ).ask()
...
def update_podcasts(all_update: bool = True) -> None:
    ...
    if not shows:
        console.print(
            Panel(
                "We have no podcasts in config, so we can't update anything.",
                style="yellow",
            )
        )
        return
    ...
    if all_update:
        console.print(
            Panel("Doing a full update of all stored podcasts...", style="magenta")
        )
...
def menu_download() -> None:
    ...
    if choice == "From stored podcasts":
        ...
        oldest = questionary.confirm(
            "Download oldest episodes first?", default=False, style=custom_style
        ).ask()

        cstr = questionary.text(
            "How many episodes do you want? (blank=all)", style=custom_style
        ).ask()
        limit = int(cstr) if cstr and cstr.isdigit() else None

        srch = questionary.text(
            "Search term to match in titles? (blank=none)", style=custom_style
        ).ask()

        log_yn = questionary.confirm(
            "Want to enable file logging?", default=False, style=custom_style
        ).ask()
        ...
        fmt = questionary.select(
            "Pick a filename format:", choices=["default", "daily"], style=custom_style
        ).ask()

        oldest = questionary.confirm(
            "Download oldest first?", default=False, style=custom_style
        ).ask()
        cstr = questionary.text(
            "How many episodes? (blank=all):", style=custom_style
        ).ask()
        limit = int(cstr) if cstr and cstr.isdigit() else None

        srch = questionary.text(
            "Title search term? (blank=none):", style=custom_style
        ).ask()
        def_out = config["system"].get("default_output_dir", DEFAULT_OUTPUT_DIR)
        outdir = questionary.text(
            "Output folder:", default=def_out, style=custom_style
        ).ask()

        log_yn = questionary.confirm(
            "Enable file logging?", default=False, style=custom_style
        ).ask()
        verb = questionary.confirm(
            "Verbose output?", default=False, style=custom_style
        ).ask()
    ...
    while True:
        ...
        if selection == "Download Podcasts":
            ...
            about_choice = questionary.select(
                "About:", choices=["Show version", "Return"], style=custom_style
            ).ask()
            ...
            console.print(
                Panel("Exiting program... Thanks for using prx!", style="bold magenta")
            )
            break
