...
Features:
  ...
  - Damaged/incomplete file checking when updating (e.g. if the app shutdown mid‑download).
...
import httpx  # For extended server info.
# NOTE: Using http2=True requires the optional "h2" package.
# To fix the error "Using http2=True, but the 'h2' package is not installed", install it via:
#    pip install h2
# or alternatively use: httpx.Client(http2=False)
...
VERSION: str = "2.0.0 [Enhanced Update, Verbose Logging, Extended Server Info & File Verification]"
...
def verify_file_integrity(file_path: str) -> bool:
    ...
    if not os.path.exists(hash_file):
        console.print(Panel(f"No hash file found for '{file_path}'.", title="File Verification", style="yellow"))
        return False
    with open(hash_file, "r") as hf:
        expected_hash = hf.read().strip()
    computed_hash = compute_sha256(file_path)
    if computed_hash == expected_hash:
        console.print(Panel(f"File verified successfully: {file_path}", title="File Verification", style="green"))
        return True
    else:
        console.print(Panel(f"File verification failed for: {file_path}", title="File Verification", style="red"))
        return False
...
def ensure_output_dir(output_dir: str) -> None:
    ...
    try:
        ...
        console.print(
            Panel(f"Error: Unable to create output directory '{output_dir}'.\n{e}", title="Error", style="red"))
        sys.exit(1)
...
def init_config() -> Tuple[configparser.ConfigParser, str]:
    ...
    if os.path.exists(config_path):
        ...
        if not QUIET_MODE:
            console.print(
                Panel(f"Config not found. Creating default config at:\n{config_path}", title="Config", style="yellow"))
...
def setup_logging() -> None:
    ...
    try:
        ...
        console.print(Panel(f"Error: Unable to create log directory '{log_dir}'.\n{e}", title="Error", style="red"))
        log_dir = ""
    timestamp: str = datetime.now().strftime('%Y%m%d_%H%M%S')
    log_file: Optional[str] = os.path.join(log_dir, f"session_{timestamp}.log") if log_dir else None
    logging.basicConfig(
    ...
    if log_file and not QUIET_MODE:
        ...
        console.print(Panel("Logging to console only (no log file directory).", title="Logging", style="yellow"))
...
def validate_config() -> None:
    ...
    if not config["user"].get("name"):
        console.print(Panel("No user name found. Let's fix that.", title="Config Update", style="yellow"))
        config["user"]["name"] = console.input("[blue]Enter your name: [/blue]")
        changed = True
    if not config["user"].get("password"):
        console.print(Panel("No password found. Let's fix that.", title="Config Update", style="yellow"))
        config["user"]["password"] = console.input("[blue]Enter your password: [/blue]")
        changed = True
    if not config["system"].get("os"):
        console.print(Panel("No OS info found. Let's fix that.", title="Config Update", style="yellow"))
        config["system"]["os"] = platform.system().lower()
        changed = True
    if changed:
        with open(config_path, "w") as f:
            config.write(f)
        console.print(Panel("Configuration updated successfully!", title="Config", style="green"))
    else:
        console.print(Panel("Configuration is already good to go.", title="Config", style="green"))
...
def manage_podcasts_in_config() -> None:
    ...
    while True:
        ...
        entries: List[str] = [chunk.strip() for chunk in podcast_line.split(";") if chunk.strip()]
        ...
        if choice == "1":
            ...
            entry: str = console.input(
                "Enter new podcast (format: PODCAST_LINK : NAME_ID : OUTPUT_DIRECTORY): ").strip()
            parts: List[str] = [p.strip() for p in entry.split(" : ")]
            if len(parts) == 3:
                podcast_triples.append((parts[0], parts[1], parts[2]))
                new_value: str = " ; ".join(
                ...
                console.print(
                    Panel("Invalid format. Please use: PODCAST_LINK : NAME_ID : OUTPUT_DIRECTORY", style="red"))
        elif choice == "3":
            if not podcast_triples:
            ...
            selection_str: str = console.input("Select podcast number to edit: ").strip()
            try:
                ...
                if 1 <= selection <= len(podcast_triples):
                    ...
                    new_entry: str = console.input(
                        "Enter new value (format: PODCAST_LINK : NAME_ID : OUTPUT_DIRECTORY, blank to keep current): ").strip()
                    if new_entry:
                        parts = [p.strip() for p in new_entry.split(" : ")]
                        if len(parts) == 3:
                            podcast_triples[selection - 1] = (parts[0], parts[1], parts[2])
                        else:
                            console.print(Panel("Invalid format. Skipping edit.", style="red"))
                            continue
                    new_value = " ; ".join(
                        [f"{link} : {name} : {out_dir}" for (link, name, out_dir) in podcast_triples])
                    config["Podcasts"]["podcast_list"] = new_value
            ...
            try:
                ...
                if 1 <= selection <= len(podcast_triples):
                    ...
                    with open(config_path, "w") as f:
                        config.write(f)
                    console.print(Panel(f"Removed podcast '{removed[1]}'", style="green"))
                else:
                    console.print(Panel("Invalid selection.", style="red"))
            except ValueError:
                console.print(Panel("Please enter a valid number.", style="red"))
...
def write_podcast_list(config: configparser.ConfigParser, podcast_triples: List[Tuple[str, str, str]]) -> None:
    """
    Write the list of tuples (podcast_link, name_id, output_directory) to the config.
    Format: "PODCAST_LINK : NAME_ID : OUTPUT_DIRECTORY ; ..."
    """
    assembled: str = " ; ".join([f"{link} : {name} : {out_dir}" for (link, name, out_dir) in podcast_triples])
...
def handle_init_command() -> None:
    ...
    """
    current_os: str = platform.system().lower()
    console.print(Panel(f"Running 'init' command for OS = {current_os}", title="Init", style="blue"))
    if "windows" in current_os:
        console.print(Panel("Current OS: WINDOWS", title="Init", style="blue"))
    elif "linux" in current_os:
        console.print(Panel("Current OS: LNX", title="Init", style="blue"))
    else:
        console.print(Panel("Unsupported OS detected. Abort.", title="Error", style="red"))
        return
    init_config()
    validate_config()
...
def manage_settings_config() -> None:
    ...
    action: str = console.input("[yellow]Choose an action (view/edit): [/yellow]").strip().lower()
    if action == "view":
        if os.path.exists(config_path):
        ...
        if edit_more == 'y':
            ...
            new_log_level: str = console.input(
...
- **Update Feature:**  
- **Update All:** Downloads every undownloaded (and/or damaged) episode for all stored podcasts.
- **Update One:** Downloads every undownloaded (and/or damaged) episode for a selected podcast.
Before downloading, the script checks if the file exists and whether it is complete (by comparing file size and/or verifying file integrity).

- **File Integrity:**  
If enabled, generates a SHA256 hash stored in a sidecar file (`.sha256`) and verifies the file after download.
...
2. **Update Podcasts:**  
 Use the "Update Podcasts" option to update all or a single show (downloads all undownloaded/damaged episodes).
3. **Settings:**  
...
def download_with_progress(url: str, output_path: str, description: str = "Downloading", verbose: bool = False,
                           timeout: int = DEFAULT_TIMEOUT, max_retries: int = DEFAULT_MAX_RETRIES,
                           initial_backoff: int = DEFAULT_INITIAL_BACKOFF) -> bool:
    """
    ...
    """
    # Display extended server info before download
    if verbose:
        ext_info = get_extended_server_info(url)
        info_str = "\n".join(f"{k}: {v}" for k, v in ext_info.items())
        console.print(Panel(f"Extended Server Info:\n{info_str}", title="Server Info", style="cyan"))

    for attempt in range(1, max_retries + 1):
        start_time: float = time.time()
        try:
            config, _ = init_config()
            https_only: bool = config["system"].getboolean("https_only", fallback=DEFAULT_HTTPS_ONLY)
            if https_only and not url.lower().startswith("https"):
                raise requests.RequestException("HTTP feeds are rejected in HTTPS-only mode.")
            ...
            if verbose:
                console.print(Panel(
                    ...
                    title="Download Speed", style="green"))
            logging.info(f"Downloaded: {url} -> {output_path}")
            # Generate and verify hash if enabled
            if config["system"].getboolean("generate_hash", fallback=DEFAULT_GENERATE_HASH):
                hash_digest: str = compute_sha256(output_path)
                hash_file: str = output_path + ".sha256"
                with open(hash_file, "w") as hf:
                    hf.write(hash_digest)
                if verbose:
                    console.print(Panel(f"SHA256 generated: {hash_digest}", title="File Integrity", style="green"))
                # Verify file integrity immediately after download
                if not verify_file_integrity(output_path):
                    console.print(
                        Panel("File verification failed. Retrying download...", title="File Verification", style="red"))
                    raise Exception("File verification failed")
            return True
        except requests.Timeout:
            console.print(
                Panel(f"Timeout on attempt {attempt}/{max_retries} for {url}", title="⚠️ Timeout", style="red"))
            logging.exception(f"Timeout on attempt {attempt} for {url}")
        except Exception as e:
            console.print(Panel(f"Error on attempt {attempt}/{max_retries} for {url}:\n{e}", title="❌ Download Error",
                                style="red"))
...
def download_podcast_rss(rss_url: str, output_dir: str, count: Optional[int] = None, searchby: Optional[str] = None,
                         verbose: bool = False, oldest_first: bool = False, format_str: str = "default") -> None:
    """
    Download episodes from a podcast RSS feed.
    For each episode, check if the corresponding file exists.
    If the file exists, compare its size to the expected length (if provided)
    and/or verify its integrity via SHA256 (if enabled). If the file is missing or damaged,
    download it.
    """
    logging.info(
    ...
    if verbose:
        console.print(
            Panel(f"Fetching RSS feed from:\n{rss_url}\nOutput directory:\n{output_dir}\nFilename Format: {format_str}",
                  title="Podcast RSS", style="blue"))
    ensure_output_dir(output_dir)
    feed = feedparser.parse(rss_url)
    if not feed.entries:
        console.print(Panel(f"No episodes found in RSS feed:\n{rss_url}", title="Podcast RSS", style="yellow"))
        logging.warning("No entries found in RSS feed.")
        return
    entries = [entry for entry in feed.entries if
               searchby and searchby.lower() in entry.title.lower()] if searchby else feed.entries
    if not entries:
        console.print(
            Panel(f"No episodes found matching search term '{searchby}'.", title="Podcast RSS", style="yellow"))
    ...
    sorted_entries = ([e for (e, _) in dated_entries_with_time] + dated_entries_without_time) if oldest_first else (
            dated_entries_without_time + [e for (e, _) in dated_entries_with_time])
    ...
    for index, entry in enumerate(entries_to_download, start=1):
        ...
        if not link:
            console.print(
                Panel(f"Skipping episode '{title}' (no downloadable link found).", title="⚠️ Skipped", style="yellow"))
        ...
        # Determine expected file size (if provided in the enclosure)
        expected_length: Optional[int] = None
        if entry.enclosures and entry.enclosures[0]:
            length_str = entry.enclosures[0].get("length", None)
            if length_str:
                try:
        ...
        # Check if file exists; if it does, check for incompleteness or damage
        if os.path.exists(file_path):
            ...
            if expected_length is not None:
                ...
                if current_size < expected_length:
                    incomplete = True
            if not incomplete and config["system"].getboolean("generate_hash", fallback=DEFAULT_GENERATE_HASH):
            ...
            if incomplete:
                console.print(
                    Panel(f"File '{file_path}' appears damaged or incomplete. Re-downloading...", title="Re-download",
                          style="yellow"))
                os.remove(file_path)
                if os.path.exists(file_path + ".sha256"):
                    os.remove(file_path + ".sha256")
            else:
                console.print(
                    Panel(f"Skipping (file exists and verified): '{file_path}'", title="⚠️ Skipped", style="yellow"))
                logging.info(f"Skipping download (already exists and complete): {file_path}")
                continue

        if verbose:
            console.print(Panel(f"Downloading episode:\nTitle: {title}\nLink: {link}\nFile: {file_path}",
                                title=f"Episode {index}/{len(entries_to_download)}", style="blue"))
        success: bool = download_with_progress(link, file_path,
                                               description=f"Episode {index}/{len(entries_to_download)}",
                                               verbose=verbose)
        if success:
            console.print(Panel(f"✔️ Successfully downloaded '{title}'", style="green"))
        else:
            console.print(Panel(f"❌ Failed to download '{title}' after {DEFAULT_MAX_RETRIES} attempts.", style="red"))
            FAILED_DOWNLOADS.append({"title": title, "url": link, "output": file_path})
        clear_screen()
    if FAILED_DOWNLOADS:
        failed_titles: str = "\n".join([f"- {item['title']}" for item in FAILED_DOWNLOADS])
        console.print(
            Panel(f"The following files failed to download:\n{failed_titles}", title="❌ Failed Downloads", style="red"))
...
def update_podcasts(all_update: bool = True) -> None:
    ...
    In update mode, the script checks every episode in the RSS feed and downloads only those that are missing or damaged.
    """
    config, _ = init_config()
    podcast_entries: List[Tuple[str, str, str]] = parse_podcast_list(config)
    if not podcast_entries:
        console.print(Panel("No stored podcasts found. Add them in Settings > Manage podcasts list.", style="yellow"))
        return
    if all_update:
        console.print(Panel("Starting batch update for all podcasts...", title="Update All", style="magenta"))
        for (plink, name_id, out_dir) in podcast_entries:
            console.print(Panel(f"Updating '{name_id}'...", title="Update", style="cyan"))
            download_podcast_rss(plink, out_dir, count=None, verbose=True)
        console.print(Panel("Batch update completed.", title="Update All", style="green"))
    else:
        ...
        matching = [(plink, name, out_dir) for (plink, name, out_dir) in podcast_entries if
                    name.lower() == name_input.lower()]
        if not matching:
            console.print(Panel(f"No podcast found with NAME_ID '{name_input}'", style="red"))
            return
        chosen_link, chosen_name, chosen_outdir = matching[0]
        console.print(Panel(f"Updating '{chosen_name}'...", title="Update", style="cyan"))
        download_podcast_rss(chosen_link, chosen_outdir, count=None, verbose=True)
        console.print(Panel(f"Update for '{chosen_name}' completed.", title="Update", style="green"))
...
def menu_manual_about() -> None:
    while True:
        ...
        if choice == "1":
            ...
            console.print(Panel(f"vrip version {VERSION}\nA podcast downloader.\nAuthor: Your Project", title="About",
                                style="green"))
        elif choice == "3":
            break
        else:
...
def menu_download() -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not podcast_entries:
                console.print(
                    Panel("No stored podcasts found. Add them in Settings > Manage podcasts list.", style="yellow"))
                continue
            ...
            oldest_first: bool = console.input("Download oldest first? (y/n): ").strip().lower() == 'y'
            count_str: str = console.input("How many episodes to download? (Leave blank for all): ").strip()
            try:
                ...
                count = None
            search_by: str = console.input("Enter search term for titles (leave blank for no filter): ").strip()
            enable_logging: bool = console.input("Enable session logging? (y/n): ").strip().lower() == 'y'
            verbose: bool = console.input("Enable verbose output? (y/n): ").strip().lower() == 'y'
            if enable_logging:
                setup_logging()
            download_podcast_rss(rss_url, output_dir, count=count, searchby=search_by, verbose=verbose,
                                 oldest_first=oldest_first, format_str=format_str)
        elif choice == "2":
            ...
            if not rss_url:
                ...
                continue
            custom_fmt: str = console.input("Format? (default/daily/...): ").strip().lower() or "default"
            oldest_first: bool = console.input("Download oldest first? (y/n): ").strip().lower() == 'y'
            count_str: str = console.input("How many episodes to download? (Leave blank for all): ").strip()
            try:
                count: Optional[int] = int(count_str) if count_str else None
            ...
            download_podcast_rss(rss_url, directory, count=count, searchby=search_by, verbose=verbose,
                                 oldest_first=oldest_first, format_str=custom_fmt)
        elif choice == "3":
            break
        else:
            console.print(Panel("Invalid choice.", title="Error", style="red"))
